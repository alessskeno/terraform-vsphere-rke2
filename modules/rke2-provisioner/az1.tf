resource "vsphere_virtual_machine" "master_nodes_az1" {
  depends_on = [vsphere_virtual_machine.template[0]]
  count                = var.master_node_count
  name                 = "${var.env}-az1-master-node-${count.index + 1}"
  ...
  clone {
    ...
    customize {
      ...
      linux_options {
        domain    = var.domain
        host_name = "${var.env}-az1-master-node-${count.index + 1}"
        time_zone = "Asia/Baku"
      }
      network_interface {
        ipv4_address = var.master_ip_range_az1[count.index]
        ipv4_netmask = 24
      }
  ...
  extra_config = {
    "guestinfo.userdata" = count.index == 0 ? base64encode(templatefile("${path.root}/files/userdata/master-node-init.yml.tpl", {
...
resource "vsphere_virtual_machine" "worker_nodes_az1" {
  depends_on = [vsphere_virtual_machine.master_nodes_az1[0]]
  count                = var.worker_node_count
  name                 = "${var.env}-az1-worker-node-${count.index + 1}"
  datastore_id         = data.vsphere_datastore.vsphere_datastore_az1.id
  host_system_id       = data.vsphere_host.vsphere_host_az1.id
  resource_pool_id     = data.vsphere_resource_pool.vsphere_resource_pool_az1.id
  ...
  clone {
    ...
    customize {
      ...
      linux_options {
        domain    = var.domain
        host_name = "${var.env}-az1-worker-node-${count.index + 1}"
        time_zone = "Asia/Baku"
      }
      network_interface {
        ipv4_address = var.worker_ip_range_az1[count.index]
        ipv4_netmask = 24
      }
      ipv4_gateway = var.vm_gw_ip_az1
    }
